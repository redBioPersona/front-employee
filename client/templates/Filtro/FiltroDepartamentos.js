...
if(Meteor.isClient){
  ...
  Template.FiltroDepartamentos.helpers({
    ultimaActualizacion:function(){
      var fecha=Sumados.findOne({"estatus" : "lastUpdateDeptos"});
      if(fecha!=undefined){
...
Template.FiltroDepartamentos.rendered = function(){
  if(Session.get('fechaInicioDepartamentos')===undefined){
    var dfIniDefaultX =moment().subtract(1, 'months').format('DD/MM/YYYY');
    $('#fechaInicio').val(dfIniDefaultX);
  }

  if(Session.get('fechaFinDepartamentos')===undefined){
    var fFinDefault = moment().format('DD/MM/YYYY');
    $('#fechaFin').val(fFinDefault);
  }

  if (Session.get('fechaFinDepartamentos')!==undefined && Session.get('fechaInicioDepartamentos')!==undefined) {
    $('#fechaFin').val(Session.get('fechaFinDepartamentos'));
    $('#fechaInicio').val(Session.get('fechaInicioDepartamentos'));
  }


}
Template.FiltroDepartamentos.events({
    ...
    if(InicialValida && FinalValida){
      ...
      if(insertar){
        ...
        if(Math.abs(difMonths)>2){
          ...
          Session.set("fechaInicioDepartamentos", Inicio);
          Session.set("fechaFinDepartamentos", Fin);
          Meteor.call("GetDepartamentos",Meteor.userId(),Inicio,Fin);

          var grafica2 = document.getElementById("grafica_deptos");
          ...
          var grafica1 = document.getElementById("grafica_deptos_emp");
